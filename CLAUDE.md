...
Granular scripts for debugging one tool at a time: `bun run typecheck`, `bun run lint:js`, `bun run lint:biome`, `bun run lint:actions`, `bun run format`, `bun run spell`, `bun run analyze`, `bun run links`, `bun run audit`, `bun run lighthouse`.
